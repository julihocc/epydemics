...
class DailyFrequencyHandler(FrequencyHandler):
    ...
    def validate_data(self, data: pd.DataFrame) -> None:
        ...
        if len(data) < 30:
            raise ValueError(f"Daily data requires at least 30 observations, got {len(data)}")
...
class WeeklyFrequencyHandler(FrequencyHandler):
    ...
    def validate_data(self, data: pd.DataFrame) -> None:
        ...
        if len(data) < 26:
            raise ValueError(f"Weekly data requires at least 26 observations (6 months), got {len(data)}")
...
class MonthlyFrequencyHandler(FrequencyHandler):
    ...
    def validate_data(self, data: pd.DataFrame) -> None:
        ...
        if len(data) < 24:
            raise ValueError(f"Monthly data requires at least 24 observations (2 years), got {len(data)}")
...
class AnnualFrequencyHandler(FrequencyHandler):
    ...
    def validate_data(self, data: pd.DataFrame) -> None:
        ...
        if len(data) < 10:
            raise ValueError(f"Annual data requires at least 10 observations (10 years), got {len(data)}")
...
def detect_frequency_from_index(date_index: pd.DatetimeIndex) -> str:
    ...
    try:
        ...
        # Thresholds: D < 1.5, W < 10, ME < 40, YE >= 40
        if avg_delta < 1.5:
            return "D"
        elif avg_delta < 10:
            return "W"
